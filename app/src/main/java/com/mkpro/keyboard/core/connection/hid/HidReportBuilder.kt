...
/**
 * USB HID *usage codes* (Usage Page 0x07) for modifier keys - distinct from
 * HidModifiers' report bitmask values above. Used when building
 * MacroStep.KeyCombo lists (e.g. Ctrl+C = [LEFT_CTRL, C]) since combos are
 * expressed as a list of usage codes, not a modifier byte.
 */
object HidModifierUsageCodes {
    const val LEFT_CTRL: Int = 0xE0
    const val LEFT_SHIFT: Int = 0xE1
    const val LEFT_ALT: Int = 0xE2
    const val LEFT_GUI: Int = 0xE3
    const val RIGHT_CTRL: Int = 0xE4
    const val RIGHT_SHIFT: Int = 0xE5
    const val RIGHT_ALT: Int = 0xE6
    const val RIGHT_GUI: Int = 0xE7
}
